					...
					<c:choose>
						<c:when test="${pointL.get_plus_mi eq true}">
							<tr style="cursor:hand; background: #e2eeff;">
								...
								<td>${pointL.point}</td>
								<c:choose>
									<c:when test="${pointL.point_get_out eq 'eve'}">
										<td>이벤트</td>
									</c:when>
									<c:when test="${pointL.point_get_out eq 'rev'}">
										<td>리뷰 작성</td>
									</c:when>
									<c:when test="${pointL.point_get_out eq 'cor'}">
										<td>코스 작성</td>
									</c:when>
									<c:when test="${pointL.point_get_out eq 'loc'}">
										<td>장소 작성</td>
									</c:when>
									<c:when test="${pointL.point_get_out eq 'com'}">
										<td>댓글 작성</td>
									</c:when>
								</c:choose>
								<td></td>
